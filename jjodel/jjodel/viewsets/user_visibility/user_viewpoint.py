...
from jjodel.jjodel.serializers.user_visibility.user_viewpoint import \
...
class ViewpointUserVisibilityViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ViewpointUserVisibility.objects.filter(viewpoint__name=vpname,
                                                      viewpoint__author__username=author)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            ViewpointUserVisibility.objects.update_or_create(user=user,
                                                            viewpoint=vp)
            ViewpointUserVisibility.objects.filter(user=user,
                                             viewpoint=vp).update(readonly=readonly)
